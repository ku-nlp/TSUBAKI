...
sub calculate_score {
    my ($this, $merged_docs, $alldocs, $did2idx, $retriever, $field_type, $qid2gid, $qid2qtf, $gid2df, $d_length_buff, $q2scores, $did2idx_is_empty, $DIST, $calculateNearDpnds, $dpnds, $qid2frq) = @_;
...
# 		    # $did2pos_score{$doc->{did}} が undef の場合、$did2pos_score{$doc->{did}}->{$pos->[$i]}{score} は 0 として扱われる
			...
			$qid2frq->{$doc->{did}}{terms}{$qid2gid->{$qid_freq->{qid}}}{qinfo}{$qid_freq->{qid}} += $score->[$i];
		    }
		}
	    }
	}
	...
	while (my ($gid, $score) = each(%total_scores)) {
		...
		$qid2frq->{$did}{terms}{$gid}{frq} = $score;
		$qid2frq->{$did}{terms}{$gid}{tff} = $tff;
		$qid2frq->{$did}{terms}{$gid}{gdf} = $df;
		$qid2frq->{$did}{terms}{$gid}{idf} = $idf;
		$qid2frq->{$did}{terms}{$gid}{okp} = $okapi_score;
		$qid2frq->{$did}{length} = $dlength;
	    }
...
sub merge_docs {
    ...
    $this->calculate_score(\@merged_docs, $alldocs_words, \%did2idx, $this->{word_retriever}, 'word', $qid2gid, $qid2qtf, $gid2df, \%d_length_buff, \@q2scores, $did2idx_is_empty, $DIST, $calculateNearDpnds, $dpnds, \%qid2frq);

    $did2idx_is_empty = 0;
    $calculateNearDpnds = 0;
    $this->calculate_score(\@merged_docs, $alldocs_dpnds, \%did2idx, $this->{dpnd_retriever}, 'dpnd', $qid2gid, $qid2qtf, $gid2df, \%d_length_buff, \@q2scores, $did2idx_is_empty, $DIST, $calculateNearDpnds, undef, \%qid2frq);

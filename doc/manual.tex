...
形態素解析器JUMANを例にして説明する。

解析結果の各行は以下の情報からなる。

\begin{quote}
表記 読み 原形 品詞 品詞細分類 活用型 活用形 意味情報
\end{quote}

また、「@」から始まる行は曖昧性のある形態素を表す。
...
「\#」から始まる行はコメント、「*」から始まる行は文節、「+」から始まる行
は基本句(\ref{subsubsec:basic_phrase}節で後述)、それ以外の行は形態素の情
報を示す。

文節、基本句の行は、係り先の文節/基本句のIDとタイプ(D,P,I,A)、Feature列
からなる。

\subsubsection{基本句・内容語}
\label{subsubsec:basic_phrase}
...
同義表現についてもインデックスを作成することで，より柔軟な検索を実現して
いる．例えば，クエリとして「乳児の虫歯を予防する」が与えられた場合，「乳
児」が「赤ちゃん」と同義であること，「予防」が「防ぐ」と同義であることを
同時に考慮した検索が行われる．その結果として，「赤ちゃんの虫歯を防ぐには」
といったクエリとは異なる表現が含まれる文書であっても検索結果として得るこ
とが可能となる．

SYNGRAPHとは，柔軟マッチングを実現するための、入力文における語・句の同義・
上位下位関係を効率的に扱うためのデータ構造である\cite{syngraph}。同義・
上位下位関係は、国語辞典とウェブコーパスから自動に獲得したものを用いてい
る。
...
結果を、図\ref{syn_dpnd}に同義表現間の係り受け関係を示す．ノードはKNPの
基本句を単位として生成される。また、複数の基本句に対して一つのSynノード
が付与される場合がある。例えば、「一番 近い」という2つの基本句に対して
\verb|<|最寄り\verb|>|というSynノードが付与される。

「!!」で始まる行はその基本句に付与されるSynノード群に共通の素性を表わす。
そして、「!」で始まる行は基本ノードまたはSynノードの情報を表す。

\begin{quote}
!! (対応する基本句のID) 係り先の基本句のIDと係りタイプ feature列
\end{quote}
...
ンデックスとして抽出する．図\ref{knp_result}に示した「子ども服をせんたく
する」という文をKNPで解析した結果からは以下の単語がインデックスとして抽
出される．
...
クエリおよび文書中の単語・句の同義表現を獲得するにあたり，SynGraphの解析
結果を利用する．
%
TSUBAKIでは，SynGraphの解析結果に含まれる基本ノードおよびSyn

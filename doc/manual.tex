...
\noindent
Step 1では，Shim-Crawlerの出力からウェブページを1件ずつ抽出することを行
う．クロール結果は，Shim-Crawlerでは，クロールした複数のウェブページを
単一のファイルに集約し，gzip圧縮した形で出力される．そして，抽出された
個々のページに対して，9桁の数字からなるページIDを付与する．ページIDは，
APIを利用して各ページを取得する際に利用される．
%
続いて，抽出されたページが，日本語ウェブページかどうかの判定を行う．ク
ロール結果に日本語以外のページが含まれる場合はStep 1で付与したページID
に欠番が生じる．そのため，日本語ページ判定処理後，再度IDを振りなおしを
行う．
%
...
(i)で収集されたページのうち，以下の理由から言語情報を用いて日本語を含
まないページを排除する．

\begin{itemize}
\item (i)ではより多くの日本語ページを取得するために，utf8コードでエンコー
  ドされているページも日本語ページとして見なしているが，utf8コードは日
  本語以外の言語でも用いられるため，日本語以外のページが含まれている可
  能性がある
\item 文字エンコードは日本語でも，日本語を含まないページが存在する
\end{itemize}

\noindent
言語情報としては，日本語の助詞（「が」，「を」，「に」，「は」，「の」，
「で」）の含有率を利用し，助詞が一定以上の割合で含まれていないページは
日本語ページでないと判定する．
...
る．}．ページ内でcharset属性が指定されている場合，その値は変更しない．
...
ウェブ標準フォーマットとは，ウェブページを解析した結果の共有を目的に，
我々が提案したXML形式のフォーマットであり，ウェブページを対象とした研究
を行う上で頻繁に利用される情報を埋め込むことが可能である．
...
オリジナルのウェブページから抽出された日本語文1文に対応している．子要素
...
クスの種類および各インデックスに含まれるデータを表\ref{size}に示す．
%（サイズについては後述する．）
%
フレーズ検索や，複数の単語が同一文中で共起していることを制約にした検索
を実現するため，索引表現の出現位置および単語の出現した文ID（標準フォー
マットから抽出可能）もインデックスデータに登録される．また，索引表現
の出現位置は，通常検索，同義表現検索のどちらにおいても，文書スコアの計
算時に利用される．
%
...
以下に，インデックス作成の手順を示す．配布ツールを用いたより具体的な作
...
SYNGRAPHの解析結果に含まれる代表表記化された単語とその読みを索引表現と
...
\noindent
このようにJumanの代表表記を索引表現とすることで，「子供」と「こども」のような表記の揺れは
解消される．これにより，表記揺れに起因する検索漏れを軽減する効果が期待できる．
...
\subsection{係り受け関係インデックス}

文をKNPで解析した結果得られる係り受け関係を索引表現として登録する．★これにより，
ウェブページを単なる単語の集合でモデル化するのではなく，構造的にモデル化することが可能である．
%
...
\noindent
が索引表現として抽出される．文節間だけでなく，内容語間の係り受け関係も
...
\subsection{同義表現インデックス}

同義表現インデックスでは，SYNGRAPHの解析結果から得られるSynノードを索引表現として用いる．
...
Synノード-Synノード，Synノード-基本ノード間の係り受け関係を索引表現として
抽出する．例えば，図\ref{syn_result}に示したSynGraphの解
...
スコアを利用する．具体的には，係り先と係り元の単語・句に対する同義表現
の持つスコア同士の積を出現頻度としている．
...
制約を指定された検索キーワードを組み合わせて検索することも可能である．
